...
import org.makarid.solanaj.programs.mango.manager.MangoManager;
import org.junit.Test;
import org.makarid.solanaj.core.PublicKey;
import org.makarid.solanaj.programs.mango.model.*;
import org.p2p.solanaj.programs.mango.model.*;
...
public class MangoTest {
  ...
  public void mangoGroupTest() throws RpcException {
    LOGGER.info(
        String.format(
            ...
            "BTC_ETH_SOL_SRM_USDC", BTC_ETH_SOL_SRM_USDC_MANGO_GROUP));

    // Retrieve MangoGroup from Solana
    final AccountInfo mangoGroupAccountInfo =
        client.getApi().getAccountInfo(BTC_ETH_SOL_SRM_USDC_MANGO_GROUP);

    // Deserialize MangoGroup
    byte[] mangoGroupData =
        Base64.getDecoder().decode(mangoGroupAccountInfo.getValue().getData().get(0));
    ...
    for (int i = 0; i < 5; i++) {
      ...
      LOGGER.info(
          String.format(
              "Token Index %d (%s): %s",
              i, mangoGroup.getTokens().get(i).toBase58(), mangoIndex.toString()));

      LOGGER.info(
          String.format(
              "Deposit = %.16f, Borrow = %.16f",
              mangoIndex.getDeposit().decodeFloat(), mangoIndex.getBorrow().decodeFloat()));
    }

    LOGGER.info(
        String.format(
            "signerNonce = %d, signerKey = %s",
            mangoGroup.getSignerNonce(), mangoGroup.getSignerKey()));

    LOGGER.info(String.format("DEX Program ID = %s", mangoGroup.getDexProgramId().toBase58()));
    ...
    mangoGroup
        ...
        .forEach(
            totalDeposit -> {
              LOGGER.info(String.format("Total Deposit = %.2f", totalDeposit.decodeFloat()));
            });
    ...
    mangoGroup
        ...
        .forEach(
            totalBorrow -> {
              LOGGER.info(String.format("Total Borrow = %.2f", totalBorrow.decodeFloat()));
            });
    ...
    LOGGER.info(
        String.format(
            "srmVault = %s, admin = %s", mangoGroup.getSrmVault(), mangoGroup.getAdmin()));
    ...
    mangoGroup
        ...
        .forEach(
            borrowLimit -> {
              LOGGER.info(String.format("Borrow limit = %d", borrowLimit));
            });
  }
  ...
  public void getDefaultMangoGroupTest() {
    ...
    LOGGER.info(String.format("defaultMangoGroup = %s", defaultMangoGroup.toString()));
  }

  @Test
  public void getMangoGroupAndMarginAccountTest() {
    final PublicKey marginAccountPk =
        PublicKey.valueOf("E95EaroWhpRnEtCpjH8HsFnzrQdcwxsXtVPwS8BbsXE");
    final MangoGroup defaultMangoGroup = mangoManager.getDefaultMangoGroup();
    final MarginAccount marginAccount =
        mangoManager.getMarginAccount(marginAccountPk, defaultMangoGroup.getDexProgramId());

    LOGGER.info(String.format("Mango Group = %s", defaultMangoGroup));

    LOGGER.info(String.format("Margin Account = %s", marginAccount.toString()));
  ...
  public void mangoV3Test() {
    final MangoPerpGroup mangoPerpGroup =
        devnetMangoManager.getMangoPerpGroup(
            PublicKey.valueOf("ECAikQUnS8HGLnzGrqEYA6Daz8nRRu9GsbfLbwMfK23P"));

    LOGGER.info(String.format("Mango Account = %s", mangoPerpGroup));
  ...
  public void mangoV3PerpAccountTest() {
    ...
    LOGGER.info(String.format("mangoPerpGroup = %s", mangoPerpGroup.toString()));

    final PublicKey TEST_MANGO_PERP_PUBKEY =
        PublicKey.valueOf("Fbq2VQNuq6WW2fHj9CDrTLSmtjvMwFajS3gXuCMTRMQV");
    final MangoPerpAccount mangoPerpAccount =
        devnetMangoManager.getMangoPerpAccount(TEST_MANGO_PERP_PUBKEY);

    LOGGER.info(String.format("mangoPerpAccount = %s", mangoPerpAccount.toString()));
