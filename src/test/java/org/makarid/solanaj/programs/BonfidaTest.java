...
import org.makarid.solanaj.programs.bonfida.manager.NamingManager;
...
public class BonfidaTest {
  ...
  private final PublicKey skynetMainnetPubkey =
  ...
  public void retrieveNameFromRegistry() {
    ...
    AccountInfo testAccountInfo =
        namingManager.getAccountInfo(new PublicKey("BVk1qg1y9AJ3LkfWCpr8FkDXZZcu7muAyVgbTBDbqDwZ"));
    ...
    LOGGER.info(
        String.format("parentName = %s, owner = %s, nameClass = %s", parentName, owner, nameClass));
  ...
  public void getPublicKeyBySolDomainTest() {
    ...
    assertTrue(
        publicKey.toBase58().equalsIgnoreCase("skynetDj29GH6o6bAqoixCpDuYtWqi1rm8ZNx1hB3vq"));

    PublicKey sbfPublicKey = namingManager.getPublicKeyBySolDomain("sbf");
    LOGGER.info(String.format("sbf.sol = %s", sbfPublicKey.toBase58()));

    assertTrue(
        sbfPublicKey.toBase58().equalsIgnoreCase("2NoEcR9cC7Rn6bP9rBpky6B1eP9syyPf8FXRaf1myChv"));
  }
}
