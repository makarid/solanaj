...
import org.makarid.solanaj.core.PublicKey.ProgramDerivedAddress;
...
public class PublicKeyTest {
  ...
  public void createProgramAddress() throws Exception {
    ...
    PublicKey programAddress =
        PublicKey.createProgramAddress(
            Arrays.asList(
                new PublicKey("SeedPubey1111111111111111111111111111111111").toByteArray()),
            programId);
    assertTrue(
        programAddress.equals(new PublicKey("GUs5qLUfsEHkcMB9T38vjr18ypEhRuNWiePW2LoK4E3K")));

    programAddress =
        PublicKey.createProgramAddress(Arrays.asList("".getBytes(), new byte[] {1}), programId);
    assertTrue(
        programAddress.equals(new PublicKey("3gF2KMe9KiC6FNVBmfg9i267aMPvK37FewCip4eGBFcT")));

    // programAddress = PublicKey.createProgramAddress(Arrays.asList("☉".getBytes()), programId);
    // assertTrue(programAddress.equals(new
    // PublicKey("7ytmC1nT1xY4RfxCV2ZgyA7UakC93do5ZdyhdF3EtPj7")));

    programAddress =
        PublicKey.createProgramAddress(
            Arrays.asList("Talking".getBytes(), "Squirrels".getBytes()), programId);
    assertTrue(
        programAddress.equals(new PublicKey("HwRVBufQ4haG5XSgpspwKtNd3PC9GM9m1196uJW36vds")));

    PublicKey programAddress2 =
        PublicKey.createProgramAddress(Arrays.asList("Talking".getBytes()), programId);
  ...
  public void findProgramAddress() throws Exception {
    ...
    ProgramDerivedAddress programAddress =
        PublicKey.findProgramAddress(Arrays.asList("".getBytes()), programId);
    assertTrue(
        programAddress
            .getAddress()
            .equals(
                PublicKey.createProgramAddress(
                    Arrays.asList("".getBytes(), new byte[] {(byte) programAddress.getNonce()}),
                    programId)));
  }
  ...
  public void findProgramAddress1() throws Exception {
    ...
    ProgramDerivedAddress programAddress =
        PublicKey.findProgramAddress(
            Arrays.asList(
                new PublicKey("8VBafTNv1F8k5Bg7DTVwhitw3MGAMTmekHsgLuMJxLC8").toByteArray()),
            programId);
    assertTrue(
        programAddress
            .getAddress()
            .equals(new PublicKey("FGnnqkzkXUGKD7wtgJCqTemU3WZ6yYqkYJ8xoQoXVvUG")));
    ...
    assertTrue(
        programAddress2
            .getAddress()
            .equals(new PublicKey("GXLbx3CbJuTTtJDZeS1PGzwJJ5jGYVEqcXum7472kpUp")));
