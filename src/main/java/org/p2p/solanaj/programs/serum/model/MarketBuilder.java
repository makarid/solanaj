...
public class MarketBuilder {
  ...
  public Market build() {
    // Only lookup account info one time since it never changes (except for fees accrued, not
    // important imo)
    if (!built) {
    ...
    if (retrieveEventQueue) {
      ...
      EventQueue eventQueue =
          EventQueue.readEventQueue(
              base64EventQueue, baseDecimals, quoteDecimals, baseLotSize, quoteLotSize);
