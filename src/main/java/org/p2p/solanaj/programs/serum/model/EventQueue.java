...
public class EventQueue {
  ...
  public static EventQueue readEventQueue(
      byte[] eventQueueData,
      byte baseDecimals,
      byte quoteDecimals,
      long baseLotSize,
    ...
    for (int i = 0; i < allocLen; ++i) {
      ...
      byte[] eventData =
          Arrays.copyOfRange(eventQueueData, eventOffset, eventOffset + NODE_LAYOUT_SPAN);
      ...
      if (fill && nativeQuantityPaid > 0) {
        ...
        if (bid) {
          double priceBeforeFees =
              maker
                  ? nativeQuantityPaid + nativeFeeOrRebate
                  : nativeQuantityPaid - nativeFeeOrRebate;
          double top = priceBeforeFees * SerumUtils.getBaseSplTokenMultiplier(baseDecimals);
          double bottom =
              SerumUtils.getQuoteSplTokenMultiplier(quoteDecimals) * nativeQuantityReleased;
          float price = (float) (top / bottom);

          tradeEvent.setFloatPrice(price);
          tradeEvent.setFloatQuantity(
              (float)
                  (nativeQuantityReleased / SerumUtils.getBaseSplTokenMultiplier(baseDecimals)));

        } else {
          double priceBeforeFees =
              maker
                  ? nativeQuantityReleased - nativeFeeOrRebate
                  : nativeQuantityReleased + nativeFeeOrRebate;
          double top = priceBeforeFees * SerumUtils.getBaseSplTokenMultiplier(baseDecimals);
          ...
          tradeEvent.setFloatPrice(price);
          tradeEvent.setFloatQuantity(
              (float) (nativeQuantityPaid / SerumUtils.getBaseSplTokenMultiplier(baseDecimals)));
        }
