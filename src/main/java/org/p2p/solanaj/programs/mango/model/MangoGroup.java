package org.p2p.solanaj.programs.mango.model;
...
public class MangoGroup {
  ...
  // Offsets
  private static final int TOKENS_OFFSET = MangoUtils.ACCOUNT_FLAGS_SIZE_BYTES;
  private static final int VAULTS_OFFSET =
      TOKENS_OFFSET + (PublicKey.PUBLIC_KEY_LENGTH * NUM_TOKENS);
  private static final int INDEXES_OFFSET =
      VAULTS_OFFSET + (PublicKey.PUBLIC_KEY_LENGTH * NUM_TOKENS);
  private static final int SPOT_MARKETS_OFFSET =
      INDEXES_OFFSET + (((U64F64.U64F64_LENGTH * 2) + MangoUtils.U64_SIZE_BYTES) * NUM_TOKENS);
  private static final int ORACLES_OFFSET =
      SPOT_MARKETS_OFFSET + (PublicKey.PUBLIC_KEY_LENGTH * NUM_MARKETS);
  private static final int SIGNER_NONCE_OFFSET =
      ORACLES_OFFSET + (PublicKey.PUBLIC_KEY_LENGTH * NUM_MARKETS);
  private static final int SIGNER_KEY_OFFSET = SIGNER_NONCE_OFFSET + MangoUtils.U64_SIZE_BYTES;
  private static final int DEX_PROGRAM_ID_OFFSET = SIGNER_KEY_OFFSET + PublicKey.PUBLIC_KEY_LENGTH;
  private static final int TOTAL_DEPOSITS_OFFSET =
      DEX_PROGRAM_ID_OFFSET + PublicKey.PUBLIC_KEY_LENGTH;
  private static final int TOTAL_BORROWS_OFFSET =
      TOTAL_DEPOSITS_OFFSET + (U64F64.U64F64_LENGTH * NUM_TOKENS);
  private static final int MAINT_COLL_RATIO_OFFSET =
      TOTAL_BORROWS_OFFSET + (U64F64.U64F64_LENGTH * NUM_TOKENS);
  ...
  private static final int MINT_DECIMALS_OFFSET =
      BORROW_LIMITS_OFFSET + (MangoUtils.U64_SIZE_BYTES * NUM_TOKENS);
  ...
  public static MangoGroup readMangoGroup(byte[] data) {
    ...
    for (int i = 0; i < NUM_TOKENS; i++) {
      final PublicKey tokenPubkey =
          PublicKey.readPubkey(data, TOKENS_OFFSET + (i * PublicKey.PUBLIC_KEY_LENGTH));
    ...
    for (int i = 0; i < NUM_TOKENS; i++) {
      final PublicKey vaultPubkey =
          PublicKey.readPubkey(data, VAULTS_OFFSET + (i * PublicKey.PUBLIC_KEY_LENGTH));
    ...
    for (int i = 0; i < NUM_TOKENS; i++) {
      int counter = (i * (MangoUtils.U64_SIZE_BYTES + (2 * U64F64.U64F64_LENGTH))); // (i * 40)
      long lastUpdate = Utils.readInt64(data, INDEXES_OFFSET);

      byte[] borrow =
          Arrays.copyOfRange(
              data,
              INDEXES_OFFSET + MangoUtils.U64_SIZE_BYTES + counter,
              INDEXES_OFFSET + MangoUtils.U64_SIZE_BYTES + U64F64.U64F64_LENGTH + counter);
      ...
      byte[] deposit =
          Arrays.copyOfRange(
              ...
              INDEXES_OFFSET + MangoUtils.U64_SIZE_BYTES + U64F64.U64F64_LENGTH + counter,
              INDEXES_OFFSET
                  + MangoUtils.U64_SIZE_BYTES
                  + U64F64.U64F64_LENGTH
                  + U64F64.U64F64_LENGTH
                  + counter);

      final MangoIndex mangoIndex =
          MangoIndex.builder()
    ...
    for (int i = 0; i < NUM_MARKETS; i++) {
      final PublicKey spotMarketPubkey =
          PublicKey.readPubkey(data, SPOT_MARKETS_OFFSET + (i * PublicKey.PUBLIC_KEY_LENGTH));
    ...
    for (int i = 0; i < NUM_MARKETS; i++) {
      final PublicKey oraclePubkey =
          PublicKey.readPubkey(data, ORACLES_OFFSET + (i * PublicKey.PUBLIC_KEY_LENGTH));
    ...
    mangoGroup.setMaintCollRatio(
        new U64F64(Arrays.copyOfRange(data, MAINT_COLL_RATIO_OFFSET, INIT_COLL_RATIO_OFFSET)));

    mangoGroup.setInitCollRatio(
        new U64F64(Arrays.copyOfRange(data, INIT_COLL_RATIO_OFFSET, SRM_VAULT_OFFSET)));
    ...
    for (int i = 0; i < NUM_TOKENS; i++) {
      long borrowLimit =
          Utils.readInt64(data, BORROW_LIMITS_OFFSET + (i * MangoUtils.U64_SIZE_BYTES));
