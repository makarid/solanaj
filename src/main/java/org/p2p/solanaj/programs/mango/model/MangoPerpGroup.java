...
public class MangoPerpGroup {
  ...
  private static final int NUM_ORACLES_OFFSET =
      METADATA_OFFSET + MangoAccountMetadata.METADATA_LAYOUT_SIZE;
  private static final int TOKENS_OFFSET = NUM_ORACLES_OFFSET + MangoUtils.U64_SIZE_BYTES;
  private static final int SPOT_MARKETS_OFFSET =
      TOKENS_OFFSET + (MAX_TOKENS * MangoTokenInfo.MANGO_TOKEN_INFO_LAYOUT_SIZE);
  private static final int PERP_MARKETS_OFFSET =
      SPOT_MARKETS_OFFSET + (MAX_PAIRS * MangoSpotMarketInfo.MANGO_SPOT_MARKET_INFO_LAYOUT_SIZE);
  private static final int ORACLES_OFFSET =
      PERP_MARKETS_OFFSET + (MAX_PAIRS * MangoPerpMarketInfo.MANGO_PERP_MARKET_INFO_LAYOUT_SIZE);
  private static final int SIGNER_NONCE_OFFSET =
      ORACLES_OFFSET + (MAX_PAIRS * PublicKey.PUBLIC_KEY_LENGTH);
  ...
  public static MangoPerpGroup readMangoPerpGroup(final PublicKey publicKey, byte[] data) {
    final MangoPerpGroup mangoPerpGroup = MangoPerpGroup.builder().publicKey(publicKey).build();
    ...
    mangoPerpGroup.setNumOracles(ByteUtils.readUint64(data, NUM_ORACLES_OFFSET).longValue());

    mangoPerpGroup.setTokens(new ArrayList<>());
    for (int i = 0; i < MAX_TOKENS; i++) {
      final MangoTokenInfo mangoTokenInfo =
          MangoTokenInfo.readMangoTokenInfo(
              Arrays.copyOfRange(
                  data,
                  TOKENS_OFFSET + (i * MangoTokenInfo.MANGO_TOKEN_INFO_LAYOUT_SIZE),
                  TOKENS_OFFSET
                      + (i * MangoTokenInfo.MANGO_TOKEN_INFO_LAYOUT_SIZE)
    ...
    for (int i = 0; i < MAX_PAIRS; i++) {
      int start =
          SPOT_MARKETS_OFFSET + (i * MangoSpotMarketInfo.MANGO_SPOT_MARKET_INFO_LAYOUT_SIZE);
      int end =
          SPOT_MARKETS_OFFSET
              + (i * MangoSpotMarketInfo.MANGO_SPOT_MARKET_INFO_LAYOUT_SIZE)
              + MangoSpotMarketInfo.MANGO_SPOT_MARKET_INFO_LAYOUT_SIZE;

      final MangoSpotMarketInfo mangoSpotMarketInfo =
          MangoSpotMarketInfo.readMangoSpotMarketInfo(Arrays.copyOfRange(data, start, end));
    ...
    for (int i = 0; i < MAX_PAIRS; i++) {
      int start =
          PERP_MARKETS_OFFSET + (i * MangoPerpMarketInfo.MANGO_PERP_MARKET_INFO_LAYOUT_SIZE);
      int end =
          PERP_MARKETS_OFFSET
              + (i * MangoPerpMarketInfo.MANGO_PERP_MARKET_INFO_LAYOUT_SIZE)
              + MangoPerpMarketInfo.MANGO_PERP_MARKET_INFO_LAYOUT_SIZE;

      final MangoPerpMarketInfo mangoPerpMarketInfo =
          MangoPerpMarketInfo.readMangoPerpMarketInfo(Arrays.copyOfRange(data, start, end));
