...
public class TokenProgram extends Program {
  ...
  /**
   * Transfers an SPL token from the owner's source account to destination account. Destination
   * pubkey must be the Token Account (created by token mint), and not the main SOL address.
   *
  ...
  public static TransactionInstruction transfer(
      PublicKey source, PublicKey destination, long amount, PublicKey owner) {
  ...
  public static TransactionInstruction transferChecked(
    ...
    keys.add(new AccountMeta(source, false, true));
    // index 1 = token mint
    // (https://docs.rs/spl-token/3.1.0/spl_token/instruction/enum.TokenInstruction.html#variant.TransferChecked)
  ...
  public static TransactionInstruction closeAccount(
      final PublicKey source, final PublicKey destination, final PublicKey owner) {
    final List<AccountMeta> keys = new ArrayList<>();
