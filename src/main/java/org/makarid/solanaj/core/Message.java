...
import org.makarid.solanaj.utils.ShortvecEncoding;
  ...
  public byte[] serialize() {
    ...
    for (TransactionInstruction instruction : instructions) {
      ...
      for (int i = 0; i < keysSize; i++) {
        keyIndices[i] =
            (byte) findAccountIndex(keysList, instruction.getKeys().get(i).getPublicKey());
      }

      CompiledInstruction compiledInstruction = new CompiledInstruction();
      compiledInstruction.programIdIndex =
          (byte) findAccountIndex(keysList, instruction.getProgramId());
