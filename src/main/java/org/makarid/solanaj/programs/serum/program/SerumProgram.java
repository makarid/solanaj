package org.makarid.solanaj.programs.serum.program;
...
import org.makarid.solanaj.programs.Program;
...
public class SerumProgram extends Program {
  ...
  /**
   * Builds a {@link TransactionInstruction} to match orders for a given Market and limit. Might not
   * be needed in Serum v3, since request queue handling changed.
   *
  ...
  public static TransactionInstruction matchOrders(Market market, int limit) {
    ...
    byte[] transactionData = encodeMatchOrdersTransactionData(limit);

    return createTransactionInstruction(
        SerumUtils.SERUM_PROGRAM_ID_V3, accountMetas, transactionData);
  }
   ...
   * @param openOrders open orders pubkey associated with this Account and market - look up using
   *     {@link SerumUtils}
   * @param market loaded market that we are trading on. this must be built by a {@link
   *     MarketBuilder}
  ...
  public static TransactionInstruction placeOrder(
      Account account, PublicKey payer, PublicKey openOrders, Market market, Order order) {
  ...
  public static TransactionInstruction placeOrder(
    ...
    byte[] transactionData = buildNewOrderv3InstructionData(order);
   ...
   */
  public static TransactionInstruction cancelOrderByClientId(
      Market market, PublicKey openOrders, PublicKey owner, long clientId) {
    List<AccountMeta> accountMetas = new ArrayList<>();
    ...
    byte[] transactionData = encodeCancelOrderByClientIdTransactionData(clientId);
   ...
   * @param clientOrderId byte array containing the clientOrderId - retrieved from an {@link
   *     OpenOrdersAccount}
   * @return {@link TransactionInstruction} for the cancelOrderByClientIdV2 call
   */
  public static TransactionInstruction cancelOrder(
      Market market, PublicKey openOrders, PublicKey owner, SideLayout side, byte[] clientOrderId) {
    List<AccountMeta> accountMetas = new ArrayList<>();
    ...
    byte[] transactionData = encodeCancelOrderTransactionData(side, clientOrderId);
  ...
  /**
   * Encodes the clientOrderId and SideLayout params used in cancelOrderV2 instructions into a byte
   * array
   *
   * @param side side of the order - buy or sell
   ...
   */
  public static TransactionInstruction settleFunds(
      Market market,
      PublicKey openOrders,
      PublicKey owner,
  ...
  /**
   * Builds a {@link TransactionInstruction} to call Consume Events for a given market and {@link
   * PublicKey}s
   *
   ...
   */
  public static TransactionInstruction consumeEvents(
      PublicKey signer,
    ...
    int limit = 5;
    byte[] transactionData = encodeConsumeEventsTransactionData(limit);
