package org.makarid.solanaj.programs.serum.manager;

import org.makarid.solanaj.programs.serum.model.*;
import org.p2p.solanaj.programs.serum.model.*;
import org.makarid.solanaj.programs.serum.program.SerumProgram;
...
import org.makarid.solanaj.programs.SystemProgram;
...
public class SerumManager {
  ...
  /**
   * Places order at the specified {@link Market} with the given {@link Order} This method looks up
   * the open orders account each time, which slows it down.
   *
  ...
  public String placeOrder(
      Account account, Market market, Order order, PublicKey baseWallet, PublicKey quoteWallet) {
    validateOrder(order);

    final OpenOrdersAccount openOrders =
        SerumUtils.findOpenOrdersAccountForOwner(
            client, market.getOwnAddress(), account.getPublicKey());

    return placeOrderInternal(account, market, order, baseWallet, quoteWallet, openOrders, null);
  }

  /**
   * Places order at the specified {@link Market} with the given {@link Order} This overloaded
   * version takes in an {@link OpenOrdersAccount}, to skip the lookup step
   *
   * @param account private key for the signer
   * @param market market being traded on
   * @param order order containing all required details
   * @param baseWallet base wallet to settle funds, used in IoC orders
   * @param quoteWallet quote wallet to settle funds, used in IoC orders
   * @param openOrdersAccount pre-determined open orders account, use {@link SerumUtils} to
   *     determine
   * @return Solana transaction ID
   */
  public String placeOrder(
    ...
    return placeOrderInternal(
        account, market, order, baseWallet, quoteWallet, openOrdersAccount, null);
  }

  /**
   * Places order at the specified {@link Market} with the given {@link Order} This overloaded
   * version takes in an {@link OpenOrdersAccount}, to skip the lookup step Allows a
   * feeDiscountPubkey to be defined for SRM fee discount
   *
   * @param account private key for the signer
   * @param market market being traded on
   * @param order order containing all required details
  ...
  public String placeOrder(
    ...
    return placeOrderInternal(
        account, market, order, baseWallet, quoteWallet, openOrdersAccount, srmFeeDiscount);
  }
  ...
  private String placeOrderInternal(
    ...
    boolean shouldWrapSol =
        (order.isBuy() && market.getQuoteMint().equals(SerumUtils.WRAPPED_SOL_MINT))
            || (!order.isBuy() && market.getBaseMint().equals(SerumUtils.WRAPPED_SOL_MINT));

    Account payerAccount = null;
    if (shouldWrapSol) {
      long lamports =
          SerumUtils.getLamportsNeededForSolWrapping(
              order.getFloatPrice(), order.getFloatQuantity(), order.isBuy(), openOrdersAccount);
      ...
      transaction.addInstruction(
          TokenProgram.initializeAccount(
              payerAccount.getPublicKey(), SerumUtils.WRAPPED_SOL_MINT, account.getPublicKey()));
    }
    ...
    final PublicKey payerPublicKey =
        shouldWrapSol ? payerAccount.getPublicKey() : (order.isBuy() ? quoteWallet : baseWallet);

    transaction.addInstruction(
        SerumProgram.placeOrder(
            account, payerPublicKey, openOrdersAddress, market, order, srmFeeDiscount));

    if (shouldWrapSol) {
      transaction.addInstruction(
          TokenProgram.closeAccount(
              payerAccount.getPublicKey(), account.getPublicKey(), account.getPublicKey()));
    }
    ...
    if (order.getOrderTypeLayout().getValue() == OrderTypeLayout.IOC.getValue()) {
      transaction.addInstruction(
          SerumProgram.settleFunds(
              market, openOrdersAddress, account.getPublicKey(), baseWallet, quoteWallet));
    }
   ...
   * @return Solana transaction ID
   */
  private String settleFundsInternal(
      Market market,
      Account account,
      PublicKey baseWallet,
      PublicKey quoteWallet,
    ...
    boolean shouldWrapSol =
        market.getQuoteMint().equals(SerumUtils.WRAPPED_SOL_MINT)
    ...
    if (shouldWrapSol) {
      transaction.addInstruction(
          TokenProgram.closeAccount(
              wrappedSolAccount.getPublicKey(), account.getPublicKey(), account.getPublicKey()));
    }
  ...
  public String consumeEvents(
    ...
    transaction.addInstruction(
        SerumProgram.consumeEvents(
            account.getPublicKey(), openOrdersAccounts, market, baseWallet, quoteWallet));
   ...
   * @param openOrdersAccount open orders account that has already been looked up using {@link
   *     SerumUtils}
   ...
   * @return Solana transaction ID
   */
  public String cancelOrderByClientIdAndSettle(
      Account owner,
      Market market,
      long clientId,
      OpenOrdersAccount openOrdersAccount,
    ...
    if (shouldWrapSol) {
      ...
      transaction.addInstruction(
          TokenProgram.initializeAccount(
              wrappedSolAccount.getPublicKey(), SerumUtils.WRAPPED_SOL_MINT, owner.getPublicKey()));
    }
    ...
    transaction.addInstruction(
        SerumProgram.cancelOrderByClientId(
            market, openOrdersAccount.getOwnPubkey(), owner.getPublicKey(), clientId));
    ...
    if (shouldWrapSol) {
      transaction.addInstruction(
          TokenProgram.closeAccount(
              wrappedSolAccount.getPublicKey(), owner.getPublicKey(), owner.getPublicKey()));
    }
   ...
   * @return Solana transaction ID
   */
  public String cancelOrderByClientId(
      Account owner, Market market, long clientId, OpenOrdersAccount openOrdersAccount) {
    final Transaction transaction = new Transaction();
    transaction.addInstruction(
        SerumProgram.cancelOrderByClientId(
            market, openOrdersAccount.getOwnPubkey(), owner.getPublicKey(), clientId));
   ...
   * @return Solana transaction ID
   */
  public String cancelOrdersByClientId(
      Account owner,
      Market market,
      List<Long> clientIds,
      OpenOrdersAccount openOrdersAccount,
  ...
  public String cancelOrderByClientId(Account owner, Market market, long clientId) {
    ...
    final OpenOrdersAccount openOrdersAccount =
        SerumUtils.findOpenOrdersAccountForOwner(
            client, market.getOwnAddress(), owner.getPublicKey());
  ...
  public String cancelAllOrdersAndSettle(
      Account owner,
      Market market,
      OpenOrdersAccount openOrdersAccount,
      PublicKey baseWallet,
  ...
  /**
   * Cancels a Serum {@link Order} by clientOrderId clientOrderId is a unique byte array retrieved
   * from an {@link OpenOrdersAccount}
   *
   * @param owner private key of the signer
   * @param market market we are trading on
  ...
  public String cancelOrder(
    ...
    transaction.addInstruction(
        SerumProgram.cancelOrder(
            market, openOrdersAccount.getOwnPubkey(), owner.getPublicKey(), side, clientOrderId));
   ...
   */
  public String settleFunds(
      Market market, Account account, PublicKey baseWallet, PublicKey quoteWallet) {
    final OpenOrdersAccount openOrdersAccount =
        SerumUtils.findOpenOrdersAccountForOwner(
            client, market.getOwnAddress(), account.getPublicKey());
  ...
  public void setOrderPrices(Order order, Market market) {
    long longPrice = SerumUtils.priceNumberToLots(order.getFloatPrice(), market);

    long longQuantity =
        SerumUtils.baseSizeNumberToLots(
            order.getFloatQuantity(), market.getBaseDecimals(), market.getBaseLotSize());

    long maxQuoteQuantity =
        SerumUtils.getMaxQuoteQuantity(order.getFloatPrice(), order.getFloatQuantity(), market);
