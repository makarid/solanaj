...
public class SolanaBip44 {
  ...
  private byte[] getPrivateKeyFromBip44SeedWithChange(byte[] seed) {
    ...
    HdAddress purposeAddress =
        hdKeyGenerator.getAddress(masterAddress, PURPOSE, solanaCoin.getAlwaysHardened()); // 55H
    HdAddress coinTypeAddress =
        hdKeyGenerator.getAddress(purposeAddress, TYPE, solanaCoin.getAlwaysHardened()); // 501H
    HdAddress accountAddress =
        hdKeyGenerator.getAddress(coinTypeAddress, ACCOUNT, solanaCoin.getAlwaysHardened()); // 0H
    HdAddress changeAddress =
        hdKeyGenerator.getAddress(accountAddress, CHANGE, solanaCoin.getAlwaysHardened()); // 0H
