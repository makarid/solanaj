...
import io.github.makarid.solanaj.programs.pyth.utils.PythUtils;
...
public class MappingAccount {
  ...
  private static final int NEXT_MAPPING_ACCOUNT_OFFSET =
      NUM_PRODUCTS_OFFSET + (2 * PythUtils.INT32_SIZE);
  private static final int PRODUCT_ACCOUNT_KEYS_OFFSET =
      NEXT_MAPPING_ACCOUNT_OFFSET + PublicKey.PUBLIC_KEY_LENGTH;
  ...
  public static MappingAccount readMappingAccount(byte[] data) {
    final MappingAccount mappingAccount =
        MappingAccount.builder()
    ...
    for (int i = 0; i < mappingAccount.getNumProducts(); i++) {
      final PublicKey productAccountKey =
          PublicKey.readPubkey(data, PRODUCT_ACCOUNT_KEYS_OFFSET + (i * 32));
