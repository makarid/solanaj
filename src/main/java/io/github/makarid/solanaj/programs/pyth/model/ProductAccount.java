...
public class ProductAccount {
  ...
  private static final int PRODUCT_ATTRIBUTES_OFFSET =
      PRICE_ACCOUNT_KEY_OFFSET + PublicKey.PUBLIC_KEY_LENGTH;
  ...
  public static ProductAccount readProductAccount(byte[] data) {
    ...
    while (index < productAccount.getSize()) {
      ...
      if (keyLength > 0) {
        String key =
            new String(Arrays.copyOfRange(data, index, index + keyLength), StandardCharsets.UTF_8);
        index += keyLength;
        ...
        String value =
            new String(
                Arrays.copyOfRange(data, index, index + valueLength), StandardCharsets.UTF_8);
        index += valueLength;
