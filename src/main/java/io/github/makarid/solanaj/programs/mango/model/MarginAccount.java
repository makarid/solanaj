...
import static io.github.makarid.solanaj.programs.mango.model.MangoUtils.ACCOUNT_FLAGS_SIZE_BYTES;
import static io.github.makarid.solanaj.core.PublicKey.PUBLIC_KEY_LENGTH;
...
public class MarginAccount {
  ...
  public static MarginAccount readMarginAccount(final PublicKey publicKey, byte[] data) {
    ...
    final MarginAccount marginAccount = MarginAccount.builder().publicKey(publicKey).build();
    ...
    for (int i = 0; i < NUM_TOKENS; i++) {
      marginAccount
          .getDeposits()
          .add(U64F64.readU64F64(data, DEPOSITS_OFFSET + (i * U64F64.U64F64_LENGTH)));
    }
    ...
    for (int i = 0; i < NUM_TOKENS; i++) {
      marginAccount
          .getBorrows()
          .add(U64F64.readU64F64(data, BORROWS_OFFSET + (i * U64F64.U64F64_LENGTH)));
    }
