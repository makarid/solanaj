...
public class RpcClient {
  ...
  public <T> T call(String method, List<Object> params, Class<T> clazz) throws RpcException {
    ...
    JsonAdapter<RpcResponse<T>> resultAdapter =
        new Moshi.Builder()
            .build()
  ...
  /**
   * Returns RPC Endpoint based on a list of weighted endpoints Weighted endpoints can be given a
   * integer weight, with higher weights used more than lower weights Total weights across all
   * endpoints do not need to sum up to any specific number
   *
    ...
    for (WeightedEndpoint endpoint : cluster.endpoints) {
      if (randomNumber > currentNumber + endpoint.getWeight()) {
        currentNumber += endpoint.getWeight();
          ...
          && randomNumber <= currentNumber + endpoint.getWeight()) {
